...
export function unitTestGreeter(): void {
  describe("Greeter", function () {
    beforeEach(async function () {
      const { greeter, erc20 } = await this.loadFixture(unitFixtureGreeter);
    ...
    it("should allow user to sendGreeting()", async function () {
      const tokenDecimals = await this.mocks.erc20.decimals();
      const amount = parseUnits("5", tokenDecimals);

      await this.mocks.erc20.mock.transferFrom
        .withArgs(this.signers.alice.address, this.contracts.greeter.address, amount)
        .returns(true);

      await expect(this.contracts.greeter.connect(this.signers.alice).sendGreeting(this.mocks.erc20.address, amount))
        .to.emit(this.contracts.greeter, "GreetingsSent")
        .withArgs(this.signers.alice.address, this.mocks.erc20.address, amount);
      ...
      expect(
        await this.contracts.greeter
          ...
          .greetings(this.signers.alice.address, this.mocks.erc20.address),
      ).to.equal(amount);
    });
    ...
    it("should revert with InsufficientGreetings() on withdrawGreeting() if sender has not sent enough greetings", async function () {
      ...
      await expect(
        this.contracts.greeter.connect(this.signers.alice).withdrawGreeting(this.mocks.erc20.address, amount),
    ...
    it("should allow user to withdrawGreeting()", async function () {
      ...
      await this.mocks.erc20.mock.transferFrom
        ...
        .returns(true);
      await this.contracts.greeter.connect(this.signers.alice).sendGreeting(this.mocks.erc20.address, sendAmount);

      await this.mocks.erc20.mock.transfer.withArgs(this.signers.alice.address, withdrawAmount).returns(true);

      await expect(
        this.contracts.greeter.connect(this.signers.alice).withdrawGreeting(this.mocks.erc20.address, withdrawAmount),
      )
        .to.emit(this.contracts.greeter, "GreetingsWithdrawn")
        .withArgs(this.signers.alice.address, this.mocks.erc20.address, withdrawAmount);
